...
try {
  ...
  for (const file of files) {
    ...
    try {
      ...
      await client.query(
        "SELECT pg_advisory_xact_lock(hashtextextended('royal-midnight-schema',0))",
      );
      const {
        rows: [exists],
      } = await client.query(
      ...
      const history =
        exists.present || apply
          ? (
              await client.query(
                "SELECT version,statements FROM supabase_migrations.schema_migrations WHERE name=$1",
                [name],
              )
            ).rows
          : [];
