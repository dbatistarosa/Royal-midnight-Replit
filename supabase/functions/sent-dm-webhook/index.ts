...
// Svix/Stripe-style tolerance: reject signed payloads whose timestamp is
// older or further in the future than this, so a captured request can't be
// replayed indefinitely even though the signature itself never expires.
...
Deno.serve(async (req: Request) => {
  ...
  if (!isTimestampFresh(timestamp)) {
    console.error("[sent-dm-webhook] timestamp outside tolerance — rejecting as stale/replayed");
    return new Response("Timestamp too old or too far in the future", { status: 400 });
  }
