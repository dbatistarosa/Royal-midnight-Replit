...
async function seedDatabase(): Promise<void> {
  try {
    ...
    if (!existing) {
      ...
      if (!seedPassword) {
        logger.error("ADMIN_SEED_PASSWORD is not set — skipping admin seed. Set this env var and restart to create the admin account.");
        return;
      }
...
// The window-based trip reminder scheduler that lived here has been removed.
// It sent emails only for bookings 55-65 minutes from pickup, which is why
// booking #6 was never notified: the GitHub Actions schedule driving the job
// fires every 40-73 minutes, so the window was narrower than the cadence and
// the run reported success while matching nothing. Its replacement is
...
function startListening(attempt = 1): void {
  ...
  server.on("listening", () => {
    ...
    // Trip reminders and driver release are NOT scheduled here.
    //
    // They live in lib/tripReminders.ts and are driven by POST /api/cron/trip-reminders,
    // which is what production actually runs: Vercel imports dist/app.mjs, so
    // nothing in this file executes there. A copy of the old window-based
    // sendTripReminders() used to be defined below and started on this interval,
    // writing the legacy reminder_sent_at column while the real module writes
    // reminder_24h_sent_at / reminder_2h_sent_at — neither deduplicating the
    // other. It never ran in production, but it read exactly like a live
    // duplicate-email bug to anyone auditing this file, and it would have become
    // one the moment this entry point was used again.
