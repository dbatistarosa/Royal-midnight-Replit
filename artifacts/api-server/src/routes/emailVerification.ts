...
router.get("/auth/email-verification", requireAuth, async (req, res) => {
  const [user] = rows<{ verified: boolean }>(
    await db.execute(
      sql`SELECT email_verified_at IS NOT NULL AS verified FROM users WHERE id=${req.currentUser!.userId}`,
    ),
  );
...
router.post(
  ...
  async (req, res) => {
    ...
    await withLock(`verify-email:${userId}`, async (tx) => {
      const [recent] = rows<{ present: boolean }>(
        await tx.execute(
          sql`SELECT true AS present FROM email_verifications WHERE user_id=${userId} AND created_at>now()-interval '1 minute'`,
        ),
      );
      if (recent) return;
     ...
     ON CONFLICT(user_id) DO UPDATE SET email=excluded.email,token_hash=excluded.token_hash,expires_at=excluded.expires_at,created_at=now()`);
      const url = new URL(
        "/auth/verify-email",
        process.env.APP_URL ?? "https://www.royalmidnight.com",
      );
      url.hash = token;
...
router.post(
  ...
  async (req, res) => {
    ...
    const ok = await withLock(
      `verify-email:${req.currentUser!.userId}`,
      async (tx) => {
        const [verification] = rows<{ email: string }>(
          await tx.execute(sql`DELETE FROM email_verifications
   WHERE user_id=${req.currentUser!.userId} AND token_hash=${hash} AND expires_at>now() RETURNING email`),
        );
        if (!verification) return false;
        const result = await tx.execute(
          sql`UPDATE users SET email_verified_at=now() WHERE id=${req.currentUser!.userId} AND email=${verification.email} RETURNING id`,
        );
        if (!rows(result).length) return false;
        await tx
          .update(bookingsTable)
          .set({ userId: req.currentUser!.userId })
          .where(
            and(
              isNull(bookingsTable.userId),
              eq(bookingsTable.passengerEmail, verification.email),
            ),
          );
        return true;
      },
