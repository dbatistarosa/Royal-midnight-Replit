...
router.get("/drivers/:id/earnings", requireAuth, async (req, res): Promise<void> => {
  ...
  const [stats] = await db
    .select({
      ...
      tipsTotal: sql<number>`coalesce(sum(tip_amount::numeric) filter (where status = 'completed'), 0)::float`,
      tipsThisWeek: sql<number>`coalesce(sum(tip_amount::numeric) filter (where status = 'completed' and created_at >= date_trunc('week', now())), 0)::float`,
      tipsToday: sql<number>`coalesce(sum(tip_amount::numeric) filter (where status = 'completed' and created_at::date = current_date), 0)::float`,
    })
    .from(bookingsTable)
