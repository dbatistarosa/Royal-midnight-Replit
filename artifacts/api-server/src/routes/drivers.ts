...
function payoutView(d: typeof driversTable.$inferSelect, log?: RequestLogger) {
  const mask = (stored: string | null): string | null => {
    try {
      ...
      log?.error(
        { err: (err as Error).message, driverId: d.id },
...
router.get("/drivers", requireAdmin, async (req, res): Promise<void> => {
  ...
  const drivers = await db
    ...
    .where(
      parsed.data.status
        ? eq(driversTable.status, parsed.data.status)
        : undefined,
    );
...
router.post("/drivers", requireAdmin, async (req, res): Promise<void> => {
  ...
  if (existingDriverRecord) {
    res
      .status(400)
      .json({ error: "A driver record with this email already exists." });
    return;
  }
  ...
  sendDriverAccountSetupEmail(email, parsed.data.name, setupLink).catch((err) =>
    req.log.error({ err }, "[drivers] account setup email failed"),
  );
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    if (caller?.role !== "admin") {
      const ownership: SQL[] = [];
      if (trackingToken)
        ownership.push(eq(bookingsTable.trackingToken, trackingToken));
      if (caller) {
        ...
        if (callerUser?.email)
          ownership.push(eq(bookingsTable.passengerEmail, callerUser.email));
      }
    ...
    const withinPhoneWindow =
      caller?.role === "admin" ||
      (!!linkedBooking &&
        ((new Date(linkedBooking.pickupAt).getTime() - Date.now()) /
          3_600_000 <=
          48 ||
...
router.patch("/drivers/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  if (parsed.data.approvalStatus != null)
    updateData.approvalStatus = parsed.data.approvalStatus;
  ...
  if (parsed.data.vehicleMake !== undefined)
    updateData.vehicleMake = parsed.data.vehicleMake ?? null;
  if (parsed.data.vehicleModel !== undefined)
    updateData.vehicleModel = parsed.data.vehicleModel ?? null;
  if (parsed.data.vehicleYear !== undefined)
    updateData.vehicleYear = parsed.data.vehicleYear ?? null;
  if (parsed.data.vehicleColor !== undefined)
    updateData.vehicleColor = parsed.data.vehicleColor ?? null;
  if (parsed.data.vehicleClass !== undefined)
    updateData.vehicleClass = parsed.data.vehicleClass ?? null;
  if (parsed.data.passengerCapacity !== undefined)
    updateData.passengerCapacity = parsed.data.passengerCapacity ?? null;
  if (parsed.data.licenseNumber !== undefined)
    updateData.licenseNumber = parsed.data.licenseNumber ?? null;
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    if (parsed.data.isOnline && driver.approvalStatus !== "approved") {
      ...
      res
        .status(403)
        .json({ error: "Driver must be approved before going online" });
      return;
    }
        ...
        if (driver) {
          db.update(driversTable)
            ...
            .catch((err) =>
              req.log.error(
                { err },
                "[drivers/by-user] retroactive userId link error",
              ),
            );
        }
      }
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const updates: Partial<typeof driversTable.$inferInsert> = {};
    if (payoutLegalName !== undefined)
      updates.payoutLegalName = payoutLegalName.trim() || null;
    if (payoutEmail !== undefined)
      updates.payoutEmail = payoutEmail.trim() || null;
    if (payoutBankName !== undefined)
      updates.payoutBankName = payoutBankName.trim() || null;
    ...
    try {
      ...
      if (routingDigits)
        updates.payoutRoutingNumber = encryptField(routingDigits);
      if (accountDigits)
        updates.payoutAccountNumber = encryptField(accountDigits);
    } catch (err) {
      if (isFieldEncryptionConfigError(err)) {
        req.log.error({ err: err.message }, "field_encryption_misconfigured");
        res.status(503).json({
          error:
            "Your banking details could not be saved securely right now. " +
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const [liveBooking] = await db
      ...
      .where(
        and(
          eq(bookingsTable.driverId, id),
          inArray(bookingsTable.status, LIVE_TRACKING_STATUSES),
        ),
      );
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const { pushToken, pushPlatform } = req.body as {
      pushToken?: string;
      pushPlatform?: string;
    };
    if (!pushToken || (pushPlatform !== "ios" && pushPlatform !== "android")) {
      res
        .status(400)
        .json({
          error: "pushToken and pushPlatform ('ios'|'android') are required",
        });
      return;
    }
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    if (driver.approvalStatus !== "approved") {
      ...
      res
        .status(403)
        .json({ error: "Driver must be approved to change availability" });
      return;
    }
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    // full + tips (100%)
    const totalEarnings =
      Math.round((commissionAllTime + tipsTotal + extrasAllTime) * 100) / 100;
    const thisWeek =
      Math.round((commissionThisWeek + tipsThisWeek + extrasThisWeek) * 100) /
      100;
    const thisMonth =
      Math.round(
    ...
    res.json(
      GetDriverEarningsResponse.parse({
        ...
        avgPerRide:
          totalRides > 0
            ? Math.round((totalEarnings / totalRides) * 100) / 100
            : 0,
        commissionAllTime,
        commissionThisWeek,
        tipsTotal,
  ...
  async (req, res): Promise<void> => {
    ...
    if (caller.role !== "admin") {
      const [callerUser] = await db
        ...
        .where(eq(usersTable.id, caller.userId));
      const emailMatch =
        callerUser?.email?.toLowerCase() === driver.email.toLowerCase();
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const { docType, fileUrl, newExpiry } = req.body as {
      docType?: string;
      fileUrl?: string;
      newExpiry?: string;
    };
    ...
    if (!docType || !validTypes.includes(docType)) {
      res
        .status(400)
        .json({ error: `docType must be one of: ${validTypes.join(", ")}` });
      return;
    }
    ...
    if (
      ...
      !/^\/objects\/[A-Za-z0-9/_.-]+$/.test(fileUrl) ||
      fileUrl.includes("..")
    ) {
      res
        .status(400)
        .json({ error: "fileUrl must be an uploaded object path" });
      return;
    }
...
function canAccessDriver(
  caller: { role: string; userId: number },
  driverUserId: number | null,
): boolean {
  return (
    caller.role === "admin" ||
    (driverUserId != null && caller.userId === driverUserId)
  );
}
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!canAccessDriver(req.currentUser!, driver.userId)) {
      req.log.warn(
        {
          ip: req.ip,
          path: req.path,
    ...
    const vehicles = await db
      ...
      .orderBy(
        desc(driverVehiclesTable.isDefault),
        driverVehiclesTable.createdAt,
      );
    res.json(vehicles);
  },
);
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    if (body.vehicleClass !== undefined)
      updateData.vehicleClass = body.vehicleClass ?? null;
    if (body.passengerCapacity !== undefined)
      updateData.passengerCapacity = body.passengerCapacity ?? null;
    if (body.luggageCapacity !== undefined)
      updateData.luggageCapacity = body.luggageCapacity ?? null;
    if (body.hasCarSeat !== undefined) updateData.hasCarSeat = body.hasCarSeat;
    if (body.regPlate !== undefined)
      updateData.regPlate = body.regPlate ?? null;
    ...
    const [v] = await db
      ...
      .where(
        and(
          eq(driverVehiclesTable.id, vehicleId),
          eq(driverVehiclesTable.driverId, driverId),
        ),
      )
      .returning();
    if (!v) {
