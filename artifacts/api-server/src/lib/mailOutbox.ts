...
export function withMailScope<T>(key: string, work: () => Promise<T>) { return scope.run(key, work); }

export async function enqueueMail(to: string | string[], subject: string, html: string, kind: string) {
  const recipient = Array.isArray(to) ? to : [to];
  const parent = scope.getStore();
  const key = parent ? crypto.createHash('sha256').update(JSON.stringify([parent,recipient,subject,kind])).digest('hex') : null;
...
type Mail = { id: string; recipient: string[]; subject: string; html: string; kind: string; attempts: number };
...
export async function drainMailOutbox() {
  ...
  for (let i=0; i<10 && Date.now()<deadline; i++) {
    const [mail] = rows<Mail>(await db.execute(sql`
    ...
    if (!mail) break;
    try {
      const payload = { from: process.env.SMTP_FROM ?? 'Royal Midnight <noreply@royalmidnight.com>',
        to: mail.recipient,subject: mail.subject,html: mail.html,replyTo: process.env.REPLY_TO ?? 'support@royalmidnight.com' };
      let providerId: string | null = null;
      if (process.env.RESEND_API_KEY) {
        const {replyTo,...content}=payload;
        const response=await fetch('https://api.resend.com/emails',{
          method:'POST',signal:AbortSignal.timeout(8000),
          headers:{Authorization:'Bearer '+process.env.RESEND_API_KEY,'Content-Type':'application/json','Idempotency-Key':mail.id},
          body:JSON.stringify({...content,reply_to:replyTo}),
        });
        const result=await response.json() as {id?:string;message?:string};
        if(!response.ok||!result.id)throw new Error(result.message??'Email provider rejected delivery');
        ...
        const transport=nodemailer.createTransport({host:process.env.SMTP_HOST,port:Number(process.env.SMTP_PORT??587),
          ...
          connectionTimeout:5000,socketTimeout:8000});
        try { providerId=(await transport.sendMail({...payload,messageId:`<${mail.id}@royalmidnight.com>`})).messageId; }
        finally { transport.close(); }
      }
      await db.execute(sql`UPDATE mail_outbox SET status='sent',sent_at=now(),lease_until=NULL,last_error=NULL,provider_id=${providerId} WHERE id=${mail.id}::uuid`);
      await db.execute(sql`INSERT INTO email_logs("to",subject,type,status) VALUES(${mail.recipient.join(', ')},${mail.subject},${mail.kind},'sent')`).catch(()=>{});
    } catch(err) {
