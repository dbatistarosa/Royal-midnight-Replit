...
export function bookingAction(handler: RequestHandler):RequestHandler {
 return async(req,res,next)=>{
  try{await withLock('payment:'+String(req.params.id ?? req.params.bookingId),async()=>{await handler(req,res,next);});}
...
export async function withLock<T>(key: string, work: (tx: Transaction) => Promise<T>): Promise<T> {
...
export async function setActor(tx: Transaction, userId?: number | null) {
  await tx.execute(sql`SELECT set_config('app.actor_user_id', ${userId ? String(userId) : ''}, true)`);
}
